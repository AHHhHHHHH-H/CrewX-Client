...
extends Command {
    ...
    public void runCommand(ArrayList<String> args) {
        if (args.size() < 3) {
            ...
            if (keyInput.equalsIgnoreCase("NONE") || keyInput.equalsIgnoreCase("NULL") || keyInput.equalsIgnoreCase("0")) {
                ...
                if (keyIndex == 0 && (buttonIndex = this.getMouseButtonIndex(keyInput)) != -1) {
    ...
    private int getMouseButtonIndex(String buttonName) {
        ...
        if (buttonName.startsWith("MOUSE")) {
            ...
            catch (NumberFormatException | StringIndexOutOfBoundsException numStr) {
                // empty catch block
            }
        }
        if ((buttonIndex = Mouse.getButtonIndex((String)buttonName)) != -1) {
            return buttonIndex;
        }
        switch (buttonName) {
            case "LBUTTON": 
