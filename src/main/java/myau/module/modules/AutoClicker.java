...
extends Module {
    ...
    private boolean isValidTarget(EntityPlayer entityPlayer) {
        if (entityPlayer != AutoClicker.mc.field_71439_g && entityPlayer != AutoClicker.mc.field_71439_g.field_70154_o) {
            if (entityPlayer == mc.func_175606_aa() || entityPlayer == AutoClicker.mc.func_175606_aa().field_70154_o) {
                return false;
            }
            if (entityPlayer.field_70725_aQ > 0) {
                return false;
            }
            float borderSize = entityPlayer.func_70111_Y();
            return RotationUtil.rayTrace(entityPlayer.func_174813_aQ().func_72314_b((double)(borderSize + ((Float)this.hitBoxHorizontal.getValue()).floatValue()), (double)(borderSize + ((Float)this.hitBoxVertical.getValue()).floatValue()), (double)(borderSize + ((Float)this.hitBoxHorizontal.getValue()).floatValue())), AutoClicker.mc.field_71439_g.field_70177_z, AutoClicker.mc.field_71439_g.field_70125_A, (double)((Float)this.range.getValue()).floatValue()) != null;
        }
        return false;
    }

    private boolean hasValidTarget() {
        return AutoClicker.mc.field_71441_e.field_72996_f.stream().filter(e -> e instanceof EntityPlayer).map(e -> (EntityPlayer)e).anyMatch(this::isValidTarget);
    }
